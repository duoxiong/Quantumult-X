/*
项目名称: GWM Auto Sign (最终修复版)
更新内容: 
1. 适配 GitHub 远程调用，移除外部依赖。
2. 修正签名算法，解决 "serverSign is not equal appSign" 报错。
3. 增加 HTTPDNS 屏蔽指引，强制 App 走域名解析以实现拦截。

---------------------------------------------------------
[Quantumult X 配置指引]

1. [general] 模块添加 (屏蔽 HTTPDNS)：
dns_exclusion_list = 119.29.29.90, 119.29.29.88, 119.28.28.28, 182.254.116.116

2. [rewrite_local] 模块添加 (请将 URL 替换为你自己的 GitHub Raw 地址)：
^https:\/\/gwm-api\.gwmapp-h\.com\/community-u\/v1\/(user\/sign\/sureNew|app\/uc\/sign\/info) url script-request-body https://raw.githubusercontent.com/你的用户名/你的仓库/main/gwm_sign.js
...
function GetCookie() {
    ...
    if (auth && gtoken) {
        ...
        // 核心：拦截包含 userId 的请求体，这是签名的关键
        if ($request.body && $request.body.includes("userId")) {
            $.setdata($request.body, KEY_BODY);
            $.msg("长城汽车", "✅ 抓取成功", "登录凭证与用户数据已同步，可随时签到。");
            console.log("✅ [GWM] Body 捕获成功: " + $request.body);
        } else {
            $.msg("长城汽车", "ℹ️ 凭证刷新", "Authorization 已更新。");
        }
    }
    $done({});
}
...
function SignIn() {
    ...
    if (!auth || !gToken || !bodyStr) {
        $.msg("长城汽车", "🚫 参数缺失", "请先打开 App 进入‘我的-每日签到’触发抓取。");
        $done(); return;
    }

    const timestamp = new Date().getTime().toString();
    // 签名公式: Secret + Body + Timestamp (必须完全一致)
    ...
    $task.fetch(myRequest).then(response => {
        try {
            ...
            if (res.code == 200 || res.success) {
                $.msg("长城汽车", "✅ 签到成功", `结果: ${res.message || "任务完成"}`);
            } else if (res.message && res.message.includes("已经签到")) {
                $.msg("长城汽车", "ℹ️ 重复签到", "今天已经签过啦！");
            } else {
                $.msg("长城汽车", "⚠️ 签到异常", `信息: ${res.message}`);
                console.log("❌ [GWM] 报错响应: " + response.body);
            }
        } catch (e) {
            $.msg("长城汽车", "❌ 响应解析失败", "服务器返回格式非 JSON");
        }
        $done();
    }, reason => {
        $.msg("长城汽车", "🚫 网络连接错误", reason.error);
        $done();
    });
}
