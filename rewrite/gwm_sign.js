...
更新内容：
1. 强化签名逻辑：强制净化 Body 格式，解决 serverSign 不匹配导致的 401 报错。
2. 域名兼容：同时支持 gwm-api 和 gapp-api。
3. 定时执行：建议设定在中午 12:00。

================ Surge 配置 ================
[MITM]
hostname = %APPEND% *.gwmapp-h.com

[Script]
获取长城汽车凭证 = type=http-request, pattern=^https:\/\/(gwm-api|gapp-api)\.gwmapp-h\.com\/community-u\/v1\/(user\/sign\/sureNew|app\/uc\/sign\/info), requires-body=1, max-size=0, script-path=https://raw.githubusercontent.com/duoxiong/Quantumult-X/refs/heads/main/rewrite/gwm_sign.js

长城汽车签到 = type=cron, cronexp=0 12 * * *, timeout=60, script-path=https://raw.githubusercontent.com/duoxiong/Quantumult-X/refs/heads/main/rewrite/gwm_sign.js, script-update-interval=0
...
[task_local]
0 12 * * * https://raw.githubusercontent.com/duoxiong/Quantumult-X/refs/heads/main/rewrite/gwm_sign.js, tag=长城汽车签到, enabled=true

================ Loon 配置 ================
[MITM]
hostname = *.gwmapp-h.com

cron "0 12 * * *" script-path=https://raw.githubusercontent.com/duoxiong/Quantumult-X/refs/heads/main/rewrite/gwm_sign.js, tag=长城汽车签到

http-request ^https:\/\/(gwm-api|gapp-api)\.gwmapp-h\.com\/community-u\/v1\/(user\/sign\/sureNew|app\/uc\/sign\/info) script-path=https://raw.githubusercontent.com/duoxiong/Quantumult-X/refs/heads/main/rewrite/gwm_sign.js, requires-body=true, timeout=10, tag=获取长城汽车凭证
...
const KEY_DOMAIN = "duoxiong_gwm_domain";
...
async function SignIn() {
    ...
    const rawBody = $.getdata(KEY_BODY);
    const domain = $.getdata(KEY_DOMAIN) || "gwm-api.gwmapp-h.com";

    if (!auth || !gToken || !rawBody) {
        $.msg($.name, "🚫 缺少数据", "请先在 App 触发抓取 (点我的/签到页)");
        $.done(); return;
    }

    // --- 核心修复：净化 Body，只保留 userId ---
    let cleanUserId = "";
    try {
        const obj = JSON.parse(rawBody);
        cleanUserId = obj.userId || (obj.data && obj.data.userId);
    } catch (e) {
    ...
    if (!cleanUserId) {
        $.msg($.name, "⚠️ 提取失败", "无法从 Body 中解析出 userId");
        $.done(); return;
    }

    // 重新构造最纯净的 Body 字符串，确保签名一致
        ...
        try {
            if (err) {
                ...
                if (res.code == 200 || res.success) {
                    $.msg($.name, "✅ 签到成功", `结果: ${res.message || "完成"}`);
                } else if (res.message && res.message.includes("已经签到")) {
                    $.msg($.name, "ℹ️ 重复签到", "今日任务已完成");
                } else {
                    $.msg($.name, "⚠️ 签名或业务错误", res.message);
                    console.log("服务器响应原文: " + data);
                }
            }
        } catch (e) {
            $.msg($.name, "❌ 响应异常", "无法解析服务器数据");
        }
        $.done();
    });
}
